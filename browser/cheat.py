...
async def main(login, password, proxy, fight, items, catch, gender, pokebol, shine, state, f=0):
    async with async_playwright() as pw:
        ...
        path = await functions.catch_pokebol(pokebol)
        ...
        while True:
            if await page.is_visible('.BtnAuth'):
                ...
                if namepok in other.runners_list and await page.is_visible("//span[contains(text(),'Можно поймать')]"):
                    f, p = await functions.runner_shine(page, namepok, f, p)  # Runner
                    ...
                    p = await functions.healing(page, pp, hp_bar, p)
